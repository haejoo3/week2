...
void Print_Line( char *line, int *Count, int *B_Flag ) {
	int i, N_Blanks, N_Chars, Start = 0; // 초기화
	...
	while ( N_Chars != 0 ) {
		if ( (*B_Flag == 1) && ((*Count + N_Chars + 1) <= LIMIT) ) {
			...
			*/
			...
			putchar(' ');
			*B_Flag = 0;
			for ( i = Start; i < Start + N_Chars; i++ ) { // 수정
				putchar(line[i]);
			}
			Start = Start + N_Chars;
			*Count = *Count + N_Chars + 1;
			...
			for ( i = Start; i < Start + N_Blanks + N_Chars; i++ ) { // 수정
				putchar(line[i]);
			}
			...
			*Count = *Count + N_Blanks + N_Chars;
			...
			putchar('\n');
			*B_Flag = 0;
			for ( i = Start + N_Blanks; i < Start + N_Blanks + N_Chars; i++ ) { // 수정
				putchar(line[i]);
			}
			Start = Start + N_Blanks + N_Chars;
			*Count = N_Chars;
